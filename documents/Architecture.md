...
- 変更が起きづらい部分に依存するように設計することで、変更に柔軟に対応できるようにする
...
src
├──pages
├──components # アプリケーション全体で再利用される汎用UIコンポーネント
│  ├──App.tsx
│  ├──index.tsx
├──hooks # カスタムフック
├──services # 外部連携
...
## なぜこの設計にしたか
- コンポーネントをページレベルと再利用可能コンポーネントに分離するため、pagesとcomponentsを分けています。
- カスタムフックを切り出して再利用可能なコンポーネントレベルの状態管理を提供できるようにしています。
- サービスレイヤーに外部連携を集約し、外部サービスと疎結合なシステムにしています。
- ユーザー情報などはコンテキストを使ってグローバルな状態管理を行えるようにしています。